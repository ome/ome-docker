...
Then either run a single all-in-one master:
    ...
    docker run -it --rm --link postgres:db -e DBUSER=postgres -e \
        DBPASS=postgres -e DBNAME=postgres --name omero-master omero-grid master
...
For example:
    ...
    docker run -it --rm --link omero-master:master \
        -v /config/master:/config:ro omero-grid master
